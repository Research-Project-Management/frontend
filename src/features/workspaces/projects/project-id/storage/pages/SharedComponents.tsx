...
import { Star, Folder, MoreVertical, Download, RotateCcw, Trash2, Pencil, FileText, Video, Music, Archive, Image as ImageIcon, File as FileIcon } from 'lucide-react';
import { motion, AnimatePresence } from 'framer-motion';
import { resolveFileUrl } from '@/features/workspaces/storage/hooks/use-blob-url';
import type { FileType } from '@/features/workspaces/storage/types/storage.types';
import { toast } from 'sonner';
import { Button, DropdownMenu, DropdownMenuContent, DropdownMenuItem, DropdownMenuSeparator, DropdownMenuTrigger } from '@/shared/components/ui';
import { DeleteModal } from '@/features/workspaces/settings';

export type StorageItem = any;
            ...
            {items.map((item) => {
              ...
              return (
                <motion.div
                  ...
                  className={`grid grid-cols-12 gap-4 items-center px-4 py-2.5 hover:bg-muted/50 cursor-pointer group transition-colors ${selectedItemId === item._id ? "bg-primary/5 border-l-2 border-l-primary" : ""
        ...
        {items.map((item) => {
          ...
          return (
            <motion.div
              ...
              className={`group bg-card border rounded-lg overflow-hidden hover:border-border hover:bg-muted/30 transition-all cursor-pointer ${selectedItemId === item._id ? "border-primary bg-primary/5 ring-1 ring-primary/20" : "border-border/50"
