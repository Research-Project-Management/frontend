...
import type { ProjectCollection } from "@/features/workspaces/library/types/library.types";
...
function PCCard({
  ...
  const activePapers = pc.papers.filter((p: any) => p.paper !== null);
  ...
  return (
    <div className="rounded-xl border border-border bg-card p-4 space-y-3 hover:shadow-sm hover:border-primary/20 transition-all duration-150">
      ...
      {activePapers.length === 0 ? (
        ...
        <div className="space-y-1.5">
          {activePapers.slice(0, 4).map((ref: any) => (
