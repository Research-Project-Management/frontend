...
import { CreateModal } from '@/features/workspaces/projects/project-id/work-items/components/modals/CreateModal';
import { useProject } from '@/features/workspaces/projects/project-id/work-items/hooks/use-work-item';
  ...
  const renderProjectItem = (project: any, keyPrefix = '') => {
    ...
    return (
      ...
      >
        ...
        {/* Project Submodules: strictly work-items, pages, cycles, views */}
        <CollapsibleContent className="overflow-hidden flex flex-col gap-1 mt-1">
          {(() => {
            ...
            // Strictly filter by MODULE_ORDER, ensuring work-items, pages, cycles, views
            const effectiveModules = MODULE_ORDER.filter(
              (k) => !projectModules || projectModules.length === 0 || activeSet.has(k)
            );
