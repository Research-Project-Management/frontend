...
import { fetchFileArrayBufferService } from "@/features/workspaces/storage/services/file-fetch.services";
import { Button } from "@/shared/components/ui";
import { Input } from "@/shared/components/ui";
import type { StorageItem } from '@/features/workspaces/storage/types/storage.types';
import { getFileType, getFileIcon, getFileColor, formatFileSize, formatDate } from './storage-views';
import { useBlobUrl, downloadFileAsBlob, resolveFileUrl } from '@/features/workspaces/storage/hooks/use-blob-url';
import { searchCrossref, lookupDoi, type CrossrefWork } from "@/features/workspaces/storage/services/crossref.services";
import { useMutation, useQueryClient } from '@tanstack/react-query';
import { updateFileMetadata } from '@/features/workspaces/storage/services/file.services';;
import { type PdfMetadata, extractDoiFromText, parseXmpMetadata, mergeCrossrefMetadata, parsePdfDate } from '@/features/editor';
  ...
  useEffect(() => {
    ...
    (async () => {
      try {
        ...
        const arrayBuffer = await fetchFileArrayBufferService(resolvedUrl);
        if (cancelled) return;
        ...
        if (!cancelled) {
          ...
          try {
            ...
            if (crossrefWork && !cancelled) {
              setMetadata((prev: any) => prev ? mergeCrossrefMetadata(prev, crossrefWork!) : prev);
...
export default function FilePreviewSidebar({ item, workspaceId, onClose, onDownload, onPreview }: FilePreviewSidebarProps) {
  ...
  const saveMetadataMutation = useMutation({
    mutationFn: (args: any) => updateFileMetadata(args.fileId, args.metaData),
    onSuccess: () => {
  ...
  const handleRetryLookup = async () => {
    ...
    try {
      ...
      if (work) {
        setMetadata((prev: any) => prev ? mergeCrossrefMetadata(prev, work!) : prev);
        setCrossrefStatus("found");
        setSaved(false);
      } else {
        setCrossrefStatus("not-found");
    ...
    <div className="w-80 h-full border-l border-border bg-background flex flex-col animate-in slide-in-from-right-4 duration-200">
      ...
      <div className="flex items-center justify-between px-4 py-3 border-b border-border">
        ...
        <button
          ...
          className="size-6 flex items-center justify-center rounded-md hover:bg-muted transition-colors text-muted-foreground outline-none focus-visible:ring-1 focus-visible:ring-ring cursor-pointer"
        >
