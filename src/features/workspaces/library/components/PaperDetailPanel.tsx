...
import { fetchLookupDoi } from '@/features/editor';
...
export function PaperDetailPanel({
  ...
  const handleCrawlDOI = async () => {
    ...
    setIsCrawling(true);
    try {
      const data = await fetchLookupDoi(paper.doi);
  ...
  return (
    <div className={cn("w-80 shrink-0 border-l border-border bg-transparent flex flex-col h-full", className)}>
        ...
        <div className="flex-1 overflow-y-auto px-4 py-3 space-y-4">
          ...
          <div className="pt-2">
            <button
              ...
              className="flex items-center justify-center gap-1.5 w-full h-9 rounded bg-primary text-primary-foreground text-xs font-semibold hover:bg-primary/95 transition-all"
            >
