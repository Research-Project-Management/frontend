...
export const ItemService = {
  ...
  getAcademicBundle: (workspaceId: string, itemId: string) =>
    ItemService.getById(workspaceId, itemId) as unknown as Promise<ItemBundle>,
  ...
  create: (workspaceId: string, collectionId: string, data: Partial<Item>) => {
  ...
  /**
   * Full replace — PUT /items/:id.
   * Conforms to PUT replacement semantics.
   */
  replace: async (
    workspaceId: string,
    itemId: string,
  ...
  setMyPublication: (workspaceId: string, itemId: string, isMyPublication: boolean) =>
    isMyPublication
      ? apiPost<{ success: boolean; data: Item; item: Item }>(
          `/api/v1/workspaces/${encodeURIComponent(workspaceId)}/library/items/${encodeURIComponent(itemId)}/my-publication`,
          {},
