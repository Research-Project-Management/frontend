...
describe('Projects Shell Utilities (projects-page.util & archive-page.util)', () => {
  describe('projects-page.util', () => {
    ...
    it('filters active vs archived projects', () => {
      const list: Partial<Project>[] = [
        { _id: '1', name: 'P1', isArchived: false },
        { _id: '2', name: 'P2', isArchived: true },
        { _id: '3', name: 'P3' },
      ];
      const active = filterActiveProjects(list as Project[]);
      expect(active).toHaveLength(2);
      expect(active.map((p) => p._id)).toEqual(['1', '3']);
    });

    it('filters projects by visibility', () => {
      const list: Partial<Project>[] = [
        { _id: '1', name: 'Public P1', isPrivate: false },
        { _id: '2', name: 'Private P2', isPrivate: true },
      ];
    ...
    it('searches projects across name, description, identifier and key', () => {
      const list: Partial<Project>[] = [
        { _id: '1', name: 'Alpha Project', description: 'Deep learning paper', identifier: 'ALP-1' },
        { _id: '2', name: 'Beta Project', description: 'Genomics workflow', key: 'GEN-2' },
      ];
    ...
    it('sorts projects by name, created, or updated date', () => {
      const list: Partial<Project>[] = [
        { _id: '1', name: 'Zeta', createdAt: '2026-01-01T00:00:00Z', updatedAt: '2026-08-01T00:00:00Z' },
        { _id: '2', name: 'Alpha', createdAt: '2026-06-01T00:00:00Z', updatedAt: '2026-08-10T00:00:00Z' },
      ];
    ...
    it('calculates project filter statistics', () => {
      const list: Partial<Project>[] = [
        { _id: '1', isPrivate: false },
        { _id: '2', isPrivate: true },
        { _id: '3', isPrivate: false },
      ];
    ...
    it('filters projects by criteria (My projects, Access, Leads, Members, Date)', () => {
      const list: Partial<Project>[] = [
        {
          ...
          isPrivate: false,
          createdBy: { _id: 'user-1', name: 'User 1' } as any,
          members: [{ role: 'lead', user: { _id: 'user-1', name: 'User 1' } }] as any,
        ...
        },
        {
          ...
          isPrivate: true,
          createdBy: { _id: 'user-2', name: 'User 2' } as any,
          members: [{ role: 'lead', user: { _id: 'user-2', name: 'User 2' } }] as any,
      ...
      expect(todayProjs[0]._id).toBe('1');
    });
  });
  ...
  describe('archive-page.util', () => {
    it('filters only archived projects', () => {
      const list: Partial<Project>[] = [
        { _id: '1', name: 'Active', isArchived: false },
        { _id: '2', name: 'Archived', isArchived: true },
      ];
    ...
    it('searches archived projects', () => {
      const list: Partial<Project>[] = [
        { _id: '1', name: 'Legacy Survey', isArchived: true },
        { _id: '2', name: 'Obsolete Benchmark', isArchived: true },
      ];
      expect(searchArchivedProjects(list as Project[], 'survey')).toHaveLength(1);
      expect(searchArchivedProjects(list as Project[], 'survey')[0]._id).toBe('1');
    });
