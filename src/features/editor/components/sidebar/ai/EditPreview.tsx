...
function DiffRow({ label, text, color }: { label: string; text: string; color: 'red' | 'green' }) {
  ...
  const bgClass = color === 'red' ? 'bg-destructive/10' : 'bg-success/10';
  const textClass =
    color === 'red'
      ? 'text-destructive line-through opacity-75 dark:text-destructive'
      : 'text-success dark:text-success';
  const gutterClass = color === 'red' ? 'text-destructive' : 'text-success';
    ...
    <>
      {lines.map((line, i) => (
        <div key={`${label}-${i}`} className={`flex px-0 ${bgClass}`}>
          <span
            className={`select-none shrink-0 w-5 text-center text-xs border-r border-border mr-2 ${gutterClass}`}
          >
            {glyph}
          </span>
...
function EditDiffBlock({
  ...
  return (
    <div className="border border-border rounded-lg overflow-hidden mb-3 text-xs bg-background ">
  ...
  return (
    <Dialog open={true} onOpenChange={(open) => !open && onCancel()}>
      <DialogContent className="w-[600px] max-w-[94vw] max-h-[85vh] p-0 flex flex-col overflow-hidden gap-0" showCloseButton={true}>
        ...
        {/* Explanation */}
        <div className="px-4 py-2.5 bg-secondary/20 border-b border-border shrink-0">
        ...
        <div className="flex-1 overflow-y-auto px-4 py-3 max-h-[50vh]">
          {isNoChange ? (
            <div className="flex flex-col items-center justify-center py-8 gap-2 text-muted-foreground/50">
              <Check className="size-8 text-success/50 shrink-0" />
              <p className="text-sm">No changes needed.</p>
            </div>
          ) : editResponse.edits.length === 0 ? (
            <div className="flex flex-col items-center justify-center py-8 gap-2 text-muted-foreground/50">
              <AlertTriangle className="size-8 text-muted-foreground/50 shrink-0" />
