...
import { fetchUser, logoutUser } from '@/features/auth';
import { useWorkspaces, fetchProjectsByWorkspaceId } from "@/features/workspaces/services/workspace.services";
  ...
  return (
    <nav className="grid w-full grid-cols-[1fr_auto] items-center gap-2 px-3 py-2 sm:grid-cols-[1fr_auto_1fr] sm:px-4">
      ...
      <div className="order-3 col-span-2 flex min-w-0 items-center justify-center gap-1.5 rounded-lg border border-border bg-background px-2 py-1.5 shadow-sm sm:order-none sm:col-span-1 sm:px-3">
        ...
        >
          <img src="/Flux.svg" alt="Flux" className="size-5" />
        </Link>
          ...
          <>
            ...
            <DropdownMenu>
              ...
              <DropdownMenuContent align="center" className="w-52">
                ...
                <DropdownMenuItem onClick={() => router.push(`/${workspaceId}`)}>
                  <PlusCircle className="mr-2 size-4" />
                  Create Project
                </DropdownMenuItem>
                <DropdownMenuSeparator />
...
export function ItemSideBar({
  ...
  const isActive = (() => {
    if (to === "") {
      // Projects: active khi khÃ´ng cÃ³ /ai, /team, /storage
      const pathAfterWorkspace = pathname.replace(
        `/${workspaceId}`,
        "",
      );
      return (
    ...
    // CÃ¡c route khÃ¡c: kiá»ƒm tra exact hoáº·c startsWith
    return (
      pathname === fullPath ||
