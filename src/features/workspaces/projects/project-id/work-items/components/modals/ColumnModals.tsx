...
import { columnFormSchema, type ColumnFormSchema } from "../../schemas/work-item.schema";
    ...
    <Dialog open={isOpen} onOpenChange={(open) => !open && onClose()}>
      <DialogContent className="sm:max-w-md gap-0 p-0 overflow-hidden border border-border shadow-sm rounded-lg">
        <form onSubmit={handleSubmit(onFormSubmit)}>
          ...
          <div className="px-6 py-4 bg-background flex flex-row items-center justify-end gap-2.5 border-t border-border">
            <Button
              type="button"
              variant="ghost"
              size="sm"
  ...
  return (
    <Dialog open={isOpen} onOpenChange={(v) => !v && onClose()}>
      ...
      <DialogContent className="max-w-[480px] p-0 overflow-hidden border border-border shadow-sm rounded-lg z-50">
        ...
        <div className="px-6 py-3.5 bg-background border-t border-border flex items-center justify-end gap-2.5">
          <Button
            type="button"
            variant="outline"
            size="sm"
