...
import { TaskDialog } from "./CardDetail";
import CreateModal, { type SectionData } from "./CreateModal";
import DeleteModal from "./DeleteModal";
import { AddExistingTaskModal } from "./AddExistingTaskModal";
import { TransferModal } from "@/features/workspaces/projects/project-id/cycles";
import { 
  useLabelsQuery,
  useProjectTasks, 
  useCreateTask, 
...
import { resolveTaskColumnId } from "@/features/workspaces/projects/project-id/tasks/types/task.types";
...
export default function Task({ cycleId, isReadOnly }: { cycleId?: string, isReadOnly?: boolean }) {
  ...
  const visibleColumns = useMemo(() => {
    ...
    const taskColumnIds = new Set(tasks.map((task: any) => task.columnId));
    return columns.filter((column: any) =>
  ...
  const findColumn = (id: string) =>
    columns.find((column: any) => resolveTaskColumnId(column) === id);
