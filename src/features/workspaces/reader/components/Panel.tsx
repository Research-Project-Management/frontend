...
import InfoSection from '@/features/workspaces/library/components/panel/InfoSection';
import AbstractSection from '@/features/workspaces/library/components/panel/AbstractSection';
import CollectionsSection from '@/features/workspaces/library/components/panel/CollectionsSection';
import NotesSection from '@/features/workspaces/library/components/panel/NotesSection';
import TagsSection from '@/features/workspaces/library/components/panel/TagsSection';
import CiteSection from '@/features/workspaces/library/components/panel/CiteSection';
import RelatedSection from '@/features/workspaces/library/components/panel/RelatedSection';

const AttachmentsSection = dynamic(() => import('@/features/workspaces/library/components/panel/AttachmentsSection'), {
  ssr: false,
});
...
import { useCollections } from '@/features/workspaces/library/hooks/use-collections';
import { useAttachments } from '@/features/workspaces/library/hooks/use-attachments';
...
import { useLibrarySidebarStore, type InspectorSectionId } from '@/features/workspaces/library/store/sidebar.store';
import { normalizeNotes, normalizeTags, convertToBibTeX, getPaperFileUrl } from '@/features/workspaces/library/utils/library.util';
import { ALL_ITEM_TYPES_FLAT } from '@/features/workspaces/library/schemas/item-type.schema';
import { cn } from "@/shared/lib/utils";
import { uploadLibraryFile } from '@/features/workspaces/library/services/upload.service';
import { toast } from 'sonner';
import { copyToClipboard } from "@/shared/lib/utils";
import { Tooltip, TooltipContent, TooltipTrigger } from "@/shared/components/ui";
import { apiPost, apiDelete } from "@/shared/lib/api";
import { useQueryClient } from '@tanstack/react-query';
import DeleteModal, { type DeleteModalConfig } from '@/features/workspaces/library/components/modals/DeleteModal';
import type { Item, Collection, CollectionInput } from '@/features/workspaces/library/types/library.types';
...
export default function InspectorPanel({
  ...
  useEffect(() => {
    if (!incomingPaper?.id) return;

    setIsPaperVerified(false);
    let cancelled = false;
    void ItemService.getById(activeWorkspaceId || 'me', incomingPaper.id)
      .then((response) => {
        if (cancelled) return;
  ...
  const handleUpdatePaper = (data: Partial<Item>) => {
    ...
    updateQueueRef.current = updateQueueRef.current
      ...
      .catch(async () => {
        ...
        try {
          const response = await ItemService.getById(activeWorkspaceId || 'me', currentPaper.id);
          const latest = response;
          if (!latest?.id) return;
          latestPaperRef.current = latest;
          setPaper((current) =>
  ...
  return (
    <div className={cn("h-full shrink-0 select-none font-sans z-10", isInspectorOpen ? "flex" : "hidden")}>
      ...
      {isInspectorOpen && (
        <>
          ...
          >
            ...
            {paper ? (
              <header className="h-9 px-3 border-b border-border bg-background flex items-center justify-between gap-2 shrink-0 select-none">
              ...
              /* Clean h-9 Header when no paper is selected */
              <header className="h-9 px-3 border-b border-border bg-background flex items-center justify-end shrink-0 select-none">
                <button
                  type="button"
                  onClick={() => setIsInspectorOpen(false)}
                  className="size-7 flex items-center justify-center rounded hover:bg-muted text-muted-foreground hover:text-foreground cursor-pointer shrink-0" title="Close panel"
