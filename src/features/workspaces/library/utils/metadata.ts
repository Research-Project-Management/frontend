...
export function mergeCrossrefMetadata(base: PdfMetadata, work: ReferenceData): PdfMetadata {
  return {
    ...base,
...
export async function extractMetadata(file: File): Promise<PdfMetadata | null> {
  ...
  try {
    ...
    if (!crossrefWork && baseMeta.title) {
      try {
        const cleanTitle = baseMeta.title.replace(/[.,;:!?\s]+$/, "");
        const res = await searchReferences(cleanTitle, 1);
        if (res && res.works && res.works.length > 0) {
          const match = res.works[0];
          // Compare titles loosely
          const matchTitle = match.title.toLowerCase().replace(/[^a-z0-9]/g, "");
          const searchTitle = cleanTitle.toLowerCase().replace(/[^a-z0-9]/g, "");
          if (matchTitle.includes(searchTitle) || searchTitle.includes(matchTitle)) {
            crossrefWork = match;
          }
        }
      } catch (err) {
