...
export const EditModal = ({
  ...
  return (
    <Dialog open={open} onOpenChange={onOpenChange}>
      <DialogContent showCloseButton={false} className="sm:max-w-[715px] flex flex-col p-0 overflow-hidden rounded-sm border-0 shadow-2xl bg-white max-h-[90vh]">
        {isReadOnly && (
        ...
        <div className="flex items-center justify-between pl-9 pr-5 py-4 border-b border-gray-100">
          <span className="text-sm font-semibold text-[#172b4d]">{isReadOnly ? 'Cycle Details' : 'Edit Cycle'}</span>
        ...
        <div className={`px-9 pt-3 pb-1 ${isReadOnly ? 'opacity-90' : ''}`}>
          <textarea
            ...
            className={`w-full resize-none rounded-md border border-transparent bg-transparent px-2 py-1 text-[24px] font-bold leading-tight outline-none placeholder:text-[#999] ${isReadOnly ? 'cursor-default' : 'hover:bg-[#091e420f] focus:bg-white focus:border-[#d9d9d9]'} text-[#172b4d] transition-all`}
            autoFocus={!isReadOnly}
          />
        </div>


        <div className={`px-9 space-y-4 max-h-[500px] overflow-y-auto custom-scrollbar pb-3 pt-1 ${isReadOnly ? 'pointer-events-none opacity-90' : ''}`}>
          ...
          <div className="flex flex-wrap items-start gap-x-6 gap-y-4">
            ...
            <div className="flex shrink-0 flex-col gap-1.5">
              ...
              <button
                ...
                className={`h-9 px-3 bg-white border border-[#d9d9d9] rounded-md flex items-center gap-2 text-[#333] font-medium text-[14px] ${isReadOnly ? 'cursor-default' : 'cursor-pointer hover:bg-[#f7f7f7]'} transition-colors`}
              >
                <PhaseIconRenderer
                  phaseId={formPhase}
                        ...
                        return (
                          <Popover key={user._id}>
                            <PopoverTrigger asChild>
                              <button className={`rounded-full ring-2 ring-white shadow-sm transition-transform ${isReadOnly ? 'cursor-default' : 'hover:scale-110 active:scale-95'} outline-none`}>
            ...
            {(formStart || formEnd) && (
              <div className="flex shrink-0 flex-col gap-1.5">
                ...
                <DatesSection formStart={formStart} formEnd={formEnd} setFormStart={setFormStart} setFormEnd={setFormEnd} trigger={
                  <div className={`inline-flex h-9 w-fit items-center gap-2 rounded-md bg-[#f1f2f4] px-3 text-[13px] font-medium text-[#172b4d] ${isReadOnly ? 'cursor-default' : 'cursor-pointer hover:bg-[#e2e4e9]'} transition-colors whitespace-nowrap`}>
          ...
          <div className="space-y-3 pt-2">
            ...
            <textarea
              ...
              className={`min-h-[100px] w-full resize-none rounded-sm border border-[#d0d7e2] px-4 py-3 text-[15px] text-[#172b4d] outline-none ${isReadOnly ? 'bg-transparent cursor-default' : 'hover:bg-[#091e420f] focus:bg-white focus:ring-2 focus:ring-[#3884ff] focus:border-[#3884ff]'} transition-all`}
            />
          </div>
        </div>
        <DialogFooter className="px-9 py-4 flex items-center justify-end gap-2 border-t border-[#ececec] bg-white shrink-0">
          {isReadOnly ? (
            <Button onClick={() => onOpenChange(false)} className="h-9 bg-black px-6 text-white hover:bg-black/90 shadow-none font-medium transition-all active:scale-95">
              Close
            </Button>
          ) : (
            <>
