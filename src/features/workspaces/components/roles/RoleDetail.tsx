...
import { useRole, useUpdateRole } from "@/features/workspaces/services/role.services";
import { useWorkspace } from "@/features/workspaces/services/workspace.services";
import {
  Shield,
  ArrowLeft,
  Save,
...
import { RESOURCES, ACTIONS, type Permission } from "@/features/workspaces/types/role.types";

export default function RoleDetailPage() {
  const { workspaceId: workspaceUrl, roleId } = useParams() as { workspaceId: string, roleId: string };
  ...
  const togglePermission = (resource: string, action: string) => {
    setFormData((prev) => {
      ...
      if (permIndex === -1) {
        // TÃ¡ÂºÂ¡o mÃ¡Â»â€ºi permission cho resource
      ...
      <div className="flex items-center justify-between">
        <div className="flex items-center gap-4">
          <Button
            variant="ghost"
            size="icon"
            onClick={() => router.push(`/${workspaceUrl}/settings/roles`)}
          >
