...
export function Lead({ members, leadId, onSelect, disabled }: LeadProps) {
  const [open, setOpen] = useState(false);
  const [search, setSearch] = useState('');

  const selected = members.find((m) => m.userId === leadId);
  ...
  return (
    <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-4">
      <div>
        <h3 className="text-sm font-medium text-foreground">Project lead</h3>
            ...
            <div className="flex items-center gap-2 min-w-0 truncate">
              {selected ? (
                ...
                <>
                  ...
                  <span className="text-muted-foreground">Select project lead</span>
                </>
              )}
            </div>
        ...
        <PopoverContent align="end" className="w-72 p-1.5 rounded-lg">
          ...
          <div className="max-h-48 overflow-y-auto space-y-0.5 pt-1">
            ...
            {filtered.length === 0 ? (
              ...
              filtered.map((m) => {
                const isSelected = m.userId === leadId;
                return (
                  <button
                    key={m.userId}
                    type="button"
