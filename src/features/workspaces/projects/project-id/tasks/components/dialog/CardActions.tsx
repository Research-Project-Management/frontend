...
export function TaskActions({
  ...
  const { data: labelsData } = useLabelsQuery(workspaceId!, 'task', projectId);
  const workspaceLabels = (labelsData as any) || [];
    ...
    <div className="mt-6 flex flex-col gap-4">
      {!isReadOnly && (
        <div className="flex flex-wrap items-center gap-2.5">
          ...
          <ActionChecklist onAddChecklist={onAddChecklist} />

          <ActionAttachment actionBtnClass={actionBtnClass} setAttachments={setAttachments} />
        </div>
      )}
        ...
        <div className="flex flex-wrap items-start gap-4 sm:gap-5">
          {assigneeId && assignee ? (
            <div className="min-w-27.5 space-y-2">
              <p className="text-[14px] font-semibold text-foreground/70">Members</p>
              <div className="flex items-center gap-2">
                <Popover open={memberActionPopoverOpen} onOpenChange={setMemberActionPopoverOpen}>
                  <PopoverTrigger asChild>
                    ...
                    >
                      <Avatar className="size-9">
                        ...
                        <AvatarFallback className="bg-muted text-[13px] font-bold text-foreground">
                  ...
                  >
                    <button
                      ...
                      className="flex w-full items-center rounded-sm px-3 py-2 text-left text-[14px] text-destructive transition-colors hover:bg-destructive/10 cursor-pointer"
                    >
                      Remove from card
                    </button>
                  </PopoverContent>
                ...
                {!isReadOnly && (
                  <Button
                    ...
                    className="size-9 rounded-full border-0 bg-muted text-foreground shadow-none hover:bg-muted/80 cursor-pointer"
          ...
          {sortedActiveLabels.length > 0 ? (
            <div className="min-w-30 space-y-2">
              <p className="text-[14px] font-semibold text-foreground/70">Labels</p>
              <div className="flex items-start gap-2">
                <div className="flex max-w-65 flex-wrap items-center gap-2">
                  {sortedActiveLabels.map((item: any) => {
                    ...
                    return (
                      <button
                        ...
                        className={
                          hasTitle
                            ? `inline-flex h-10 max-w-45 items-center rounded-sm px-4 text-[15px] leading-none font-medium transition-opacity ${isReadOnly ? 'cursor-default' : 'hover:opacity-90 cursor-pointer'}`
                            : `inline-flex h-10 w-11 items-center justify-center rounded-sm transition-opacity ${isReadOnly ? 'cursor-default' : 'hover:opacity-90 cursor-pointer'}`
                        }
                ...
                {!isReadOnly && (
                  <Button
                    ...
                    className="size-9 shrink-0 self-start rounded-full border-0 bg-muted text-foreground shadow-none hover:bg-muted/80 cursor-pointer"
          ...
          {dateBadgeText ? (
            <div className="min-w-45 space-y-2">
              ...
              >
                ...
                {isDueDateOverdue ? (
                  <span className="inline-flex h-5 items-center rounded-sm bg-destructive px-1.5 text-[12px] font-semibold text-destructive-foreground">
                    Overdue
                  </span>
                ) : null}
                {!isReadOnly && <ChevronDown className="size-4 shrink-0 text-foreground" />}
              </button>
            </div>
          ) : null}
        </div>
