...
import {
  ...
  useCollections,
  useUpdatePaper,
} from "@/features/library/services/library.services";
import { lookupDoi, searchCrossref, type CrossrefWork } from '@/features/storage';
import { resolveFileUrl } from "@/features/library/hooks/useBlobUrl";
  ...
  return (
    <div className="h-full overflow-y-auto px-4 py-4">
      ...
      <dl className="mt-5 border-y border-border">
        ...
        <MetadataField label="Part">{[paper.partNumber, paper.partTitle].filter(Boolean).join(" Â· ")}</MetadataField>
        <MetadataField label="Series">{[paper.series, paper.seriesTitle, paper.seriesText].filter(Boolean).join(" Â· ")}</MetadataField>
        ...
        <MetadataField label="Archive">{[paper.archive, paper.archiveLocation].filter(Boolean).join(" Â· ")}</MetadataField>
