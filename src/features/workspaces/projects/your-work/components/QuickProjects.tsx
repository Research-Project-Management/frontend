  ...
  return (
    <div className="w-full">
      <div className="flex items-center justify-between mb-4">
        ...
        <Link href={`/${workspaceId}`} className="text-xs font-medium text-foreground hover:text-foreground transition-colors flex items-center gap-1">
          View all
          ...
          return (
            ...
            >
              <div className="size-10 flex items-center justify-center rounded-lg bg-muted text-foreground group-hover:bg-accent transition-colors">
                <span className="text-lg">{project.avatar || <Folder className="h-5 w-5 text-foreground" />}</span>
              </div>
              ...
              <ChevronRight className="h-3.5 w-3.5 text-foreground opacity-0 -translate-x-2 group-hover:opacity-100 group-hover:translate-x-0 transition-all duration-300 shrink-0" />
            </Link>
          );
        })}
      </div>
