...
export function Topbar({
  ...
  return (
    <header
      className={cn(
        'h-12 border-b border-border px-4 flex items-center justify-between gap-3 bg-background shrink-0 text-13 w-full min-w-0 overflow-x-auto scrollbar-none',
        className,
      )}
    >
      ...
      >
          ...
          <>
            ...
            <DropdownMenu>
              <DropdownMenuTrigger asChild>
                <button
                  type="button"
                  className="flex items-center gap-1.5 h-7 px-2 rounded-md text-xs font-medium text-foreground hover:bg-muted transition-colors cursor-pointer outline-none focus-visible:ring-1 focus-visible:ring-primary shrink-0 select-none"
                >
        ...
        {/* Active Saved View / Archived Breadcrumb Context */}
        {activeSavedView && !showArchived && (
          <>
            <ChevronRight className="size-3.5 text-muted-foreground/40 shrink-0 mx-0.5" strokeWidth={1.75} />
            <span className="text-13 font-medium text-foreground/80 truncate max-w-[120px]">
