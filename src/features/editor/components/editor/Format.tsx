...
import { DropdownMenu, DropdownMenuContent, DropdownMenuItem, DropdownMenuTrigger, DropdownMenuLabel, DropdownMenuSeparator, DropdownMenuSub, DropdownMenuSubTrigger, DropdownMenuSubContent } from '@/shared/components/ui/dropdown-menu';
    ...
    <Tooltip>
      <TooltipTrigger asChild>
        <button
          ...
          className={cn(
            'h-7 min-w-7 px-1.5 flex items-center justify-center gap-1 rounded text-xs font-medium transition-all duration-150 outline-none select-none',
            'active:scale-95',
            active
      ...
      <TooltipContent side="bottom" align="center" className="flex items-center gap-2">
        <span>{tooltip}</span>
        {kbd && (
          <kbd className="bg-muted px-1 rounded text-xs text-muted-foreground font-mono leading-none border border-border">
            {kbd}
          </kbd>
        )}
      </TooltipContent>
    ...
    <TooltipProvider>
      ...
      >
        ...
        <div className="flex items-center gap-1 overflow-x-auto no-scrollbar py-0.5">
          ...
          {showFormat && (
            <div className="flex items-center gap-px px-1.5 border-r border-border">
            ...
            <div className="flex items-center gap-px px-1.5 border-r border-border">
              ...
              <DropdownMenu>
                <DropdownMenuTrigger asChild>
                  <button className="h-7 px-1.5 flex items-center justify-center gap-0.5 rounded-md text-xs font-medium text-foreground hover:bg-muted active:scale-95 outline-none focus-visible:ring-1 focus-visible:ring-primary">
          ...
          {showStructure && (
            <div className="flex items-center gap-px px-1.5">
              <DropdownMenu>
                <DropdownMenuTrigger asChild>
                  <button className="h-7 px-1.5 flex items-center justify-center gap-1 rounded-md text-xs font-medium text-foreground hover:bg-muted active:scale-95 outline-none focus-visible:ring-1 focus-visible:ring-primary">
            ...
            <DropdownMenu>
              <DropdownMenuTrigger asChild>
                <button
                  ...
                  className="h-7 w-7 flex items-center justify-center rounded-md text-foreground hover:bg-muted active:scale-95 outline-none shrink-0"
                >
                ...
                {!showMath && (
                  <>
                    ...
                    <DropdownMenuItem onClick={() => handleFormat('inlineMath')}>
                      <Sigma className="size-4 mr-2 shrink-0" />
                      <span>Inline Math</span>
                    </DropdownMenuItem>
                    <DropdownMenuItem onClick={() => handleFormat('displayMath')}>
                      <Braces className="size-4 mr-2 shrink-0" />
                ...
                {!showStructure && (
                  <>
                    ...
                    <DropdownMenuItem onClick={() => handleInsert('\\section{}')}>
                      <Type className="size-4 mr-2 shrink-0" />
                      <span>Section (H1)</span>
                    </DropdownMenuItem>
                    <DropdownMenuItem onClick={() => handleInsert('\\item ')}>
                      <List className="size-4 mr-2 shrink-0" />
                      <span>List Item</span>
                    </DropdownMenuItem>
                    <DropdownMenuItem onClick={() => handleFormat('cite')}>
                      <BookOpen className="size-4 mr-2 shrink-0" />
                      <span>Citation</span>
                    </DropdownMenuItem>
                  </>
                )}
        ...
        <div className="flex items-center gap-1.5 shrink-0 pl-3 border-l border-border">
          ...
          {showQuickSettings ? (
            <>
              ...
              <div className="flex items-center bg-muted rounded-md border border-border ml-0.5">
                <ToolbarButton
            ...
            <DropdownMenu>
              ...
              <DropdownMenuContent align="end" className="w-56 z-[9999]">
                ...
                <DropdownMenuItem onClick={() => setWordWrap(!wordWrap)}>
                  <Pilcrow className={cn('size-4 mr-2 shrink-0', wordWrap && 'text-primary')} />
                  <span>Word Wrap</span>
                ...
                <DropdownMenuItem onClick={() => setLineNumbers(!lineNumbers)}>
                  <Hash className={cn('size-4 mr-2 shrink-0', lineNumbers && 'text-primary')} />
