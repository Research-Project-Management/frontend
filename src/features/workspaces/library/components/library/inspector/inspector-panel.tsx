...
import { getLibraryEntityId, getPaperFileUrl, normalizeNotes } from '../../../utils/library.util';
...
export default function InspectorPanel({
  ...
  useEffect(() => {
    ...
    const handleMouseMove = (e: MouseEvent) => {
      // Left-side dragging: pulling left increases width
  ...
  const paperFileUrl = getPaperFileUrl(paper);
  ...
  const handleAddNote = (content: string) => {
    if (!paperId) return;
    const existingNotes = normalizeNotes(paper.notes).map((n) => n.content);
    paperService.actions.updatePaper(
      {
        paperId,
  ...
  const handleDeleteNote = (noteId: string) => {
    ...
    const updated = normalized.filter((n) => n.id !== noteId).map((n) => n.content);
    paperService.actions.updatePaper(
      {
        paperId,
  ...
  const handleUpdateNote = (noteId: string, content: string) => {
    if (!paperId) return;
    const normalized = normalizeNotes(paper.notes);
    const updated = normalized.map((n) => (n.id === noteId ? content : n.content));
    paperService.actions.updatePaper(
      {
        paperId,
        notes: updated as any,
  ...
  const tagsCount = paper.labels?.length || 0;
    ...
    >
      ...
      <div className="h-14 px-4 border-b border-border/50 flex items-center justify-between shrink-0 bg-background/80 backdrop-blur-md">
        ...
        <div className="flex items-center gap-1 shrink-0">
          <button
            onClick={() => router.push(`/${workspaceUrl}/library/papers/${paper.id || paper._id}`)}
      ...
      <div className="flex items-center border-b border-border/30 bg-muted/20 px-2 py-1 gap-1 shrink-0 overflow-x-auto">
        {TABS.map((tab) => {
          ...
          const count = tab.id === 'notes' ? notesCount : 0;
          ...
          return (
            <button
              ...
              className={cn(
                'relative flex items-center gap-1.5 px-2.5 py-1 rounded-md text-xs font-medium transition-all cursor-pointer',
                isActive
