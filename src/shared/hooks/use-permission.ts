...
export function useProjectPermission(
  project?: {
    ...
    members?: Array<{ userId?: string | { id?: string; _id?: string }; role: string }>;
  } | null,
  ...
  return useMemo(() => {
    ...
    const member = project.members?.find((m) => {
      const mId = typeof m.userId === 'object' ? (m.userId?.id || (m.userId as any)?._id) : m.userId;
      return mId === userId;
    });
