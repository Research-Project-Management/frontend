...
import type { Relation, RelationType, Item } from '../../types/work-item.types';
                ...
                <div className="space-y-1">
                  ...
                  <div className="max-h-40 overflow-y-auto divide-y divide-border rounded-md border border-border mt-1 bg-background">
                    {selectableItems.length === 0 ? (
                      ...
                      selectableItems.slice(0, 20).map((t) => {
                        const isChosen = selectedTargetItem?.id === t.id;
                        return (
                          <button
                            key={t.id}
                            type="button"
                            onClick={() => setSelectedTargetItem(t)}
                            className={cn(
