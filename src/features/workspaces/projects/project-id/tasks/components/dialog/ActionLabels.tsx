...
export interface ActionLabelsProps {
  labels: string[];
...
export const ActionLabels: React.FC<ActionLabelsProps> = ({ 
  labels, 
  setLabels,
  actionBtnClass,
  open: externalOpen,
      ...
      return (
        <div className="flex h-full min-h-0 flex-col bg-white">
          ...
          <div className="flex shrink-0 items-center justify-between border-t border-border/50 bg-white p-3">
            ...
            {editingLabelId && (
              <Button 
                onClick={() => handleDelete((id: any) => setLabels(prev => prev.filter(p => p !== id)))} 
                variant="destructive" 
                disabled={isMutating}
              ...
              filteredLabels.map((label: any) => (
                <div key={label._id} className="grid h-10 grid-cols-[20px_minmax(0,1fr)_32px] items-center gap-3 group">
                  ...
                  <Button 
                    ...
                    className="size-8 shrink-0 opacity-60 transition-opacity group-hover:opacity-100 cursor-pointer"
  ...
  return (
    <Popover open={open} onOpenChange={(val) => { onOpenChange(val); if(!val) setView("list"); }}>
      <PopoverTrigger asChild>
        <button className={cn(
          "flex h-10 w-fit items-center gap-3 rounded-md px-4 text-sm font-medium transition-colors hover:bg-muted/50 cursor-pointer",
          actionBtnClass
        )}>
      ...
      <PopoverContent
        ...
        className="z-[1000] flex w-80 min-h-0 flex-col overflow-hidden rounded-xl border-border/50 p-0 shadow-xl bg-popover"
...
export const ActionLabelsSection = ActionLabels;
