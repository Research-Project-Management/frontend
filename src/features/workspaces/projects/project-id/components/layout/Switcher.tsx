...
export function Switcher({
  ...
  return (
    <div className={cn('flex items-center gap-2 min-w-0 shrink select-none', className)}>
      {/* 1. Project Switcher with Arrow Button (matches user's sample image) */}
      <Popover open={open} onOpenChange={setOpen}>
        <div className="flex items-center gap-1 shrink-0">
          <button
            ...
            onClick={() => setOpen((v) => !v)}
            className={cn(
              'flex items-center gap-1.5 h-7 px-2 rounded-md text-13 font-medium text-foreground transition-colors cursor-pointer outline-none focus-visible:ring-1 focus-visible:ring-primary shrink-0 select-none',
          ...
          <PopoverTrigger asChild>
            <button
              ...
              className={cn(
                'flex items-center justify-center size-7 rounded-md text-foreground transition-colors cursor-pointer outline-none focus-visible:ring-1 focus-visible:ring-primary shrink-0 select-none',
                open ? 'bg-muted' : 'bg-muted/60 hover:bg-muted',
              )}
            >
      ...
      {/* 2. Module Title (Clean, NO dropdown menu as requested) */}
      <div className="flex items-center gap-1.5 h-7 px-1 text-13 font-medium text-foreground select-none shrink-0">
