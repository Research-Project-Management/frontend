      ...
      <div className="flex-1 min-w-0">
        ...
        {(entry.file || entry.line !== undefined) && (
          <p className="text-[10px] mt-0.5 text-zinc-500">
            ...
            {entry.file && entry.line !== undefined && <span> Â· </span>}
...
function CompileButton({
  ...
  compileStatus: import("@/features/editor/store/compile.store").CompileStatus;
  onCompile: () => void;
  engine: LaTeXEngine;
  ...
  const handleDoubleClickPage = (pageNum: number, clickFraction: number) => {
    ...
    if (smap && approxTag > 1) {
      ...
      if (filepath) {
        ...
        if (matchingPage && matchingPage._id !== activeFilePage) {
          setActiveFilePage(matchingPage._id);
