...
import { useSticky } from '@/features/workspaces/projects/project-id/stickies/hooks/use-sticky';
import { type Sticky, STICKY_COLOR_CYCLE } from '@/features/workspaces/projects/project-id/stickies/types/sticky.types';
...
export const useCard = (options?: { search?: string }) => {
  ...
  const api = useSticky(projectId, search);
  ...
  return {
    ...
    actions: {
      add: useCallback(() => {
        if (!projectId || api.mutations.create.isPending) return;
      ...
      }, [projectId, stickies, api.mutations.create]),
