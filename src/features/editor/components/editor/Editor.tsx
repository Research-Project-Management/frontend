...
export default function Editor({ page }: EditorProps) {
  ...
  const { pageId: pageIdParam, projectId: projectIdParam } = useParams<{ pageId?: string; projectId?: string }>();
  ...
  const { data: libraryData } = useViewItems(projectIdRef.current, 'all');
  ...
  useEffect(() => {
    ...
    if (debouncedPayload.text !== currentSavedText) {
      markDirty(currentPage.id, debouncedPayload.text);
