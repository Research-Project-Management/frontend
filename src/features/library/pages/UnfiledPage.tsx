...
export default function UnfiledPage() {
  ...
  const { data: viewData, isLoading } = useViewItems(effectiveScopeId || workspaceId, 'unfiled', search);
  ...
  return (
    <div className="flex h-full min-w-0 flex-1 overflow-hidden font-sans">
      ...
      <div className="flex flex-col flex-1 min-w-0 h-full overflow-hidden">
        ...
        {/* Central Items Table - Managed Directly by UnfiledPage */}
        <div className="flex-1 min-h-0 overflow-hidden flex flex-col bg-background">
          {isLoading && unfiledItems.length === 0 ? (
            ...
            <div className="flex flex-col items-center justify-center flex-1 h-full min-h-[300px] text-center p-8 select-none">
              <div className="size-12 rounded-full bg-muted flex items-center justify-center mb-3">
                <Inbox className="size-6 text-foreground shrink-0" />
              </div>
              <h2 className="text-sm font-semibold text-foreground">No unfiled items</h2>
              <p className="text-xs text-muted-foreground mt-1 max-w-sm">
                {search.trim()
                  ? 'No unfiled references matching your search query.'
                  : 'All references in your library have been organized into collections.'}
              </p>
              {search.trim() && (
                <button
                  type="button"
                ...
                <thead className="sticky top-0 z-20 bg-background border-b border-border select-none">
                  <tr className="h-9 type-dense font-normal text-foreground [&_th]:font-normal [&_th]:text-foreground">
                    ...
                    <th
                      ...
                      className="group/th px-3.5 py-1.5 align-middle cursor-pointer outline-none focus-visible:ring-1 focus-visible:ring-primary focus-visible:ring-inset select-none"
                    >
                      <div className="flex items-center">
                        <span>Year</span>
                        {renderSortIcon('year')}
                    ...
                    return (
                      <ContextMenu key={paper.id}>
                        ...
                        <ContextMenuContent className="w-56 p-1.5 text-12 font-sans rounded-md border border-border bg-popover text-popover-foreground shadow-raised-200 space-y-0.5">
                          ...
                          <ContextMenuItem onClick={() => handleInitiateSingleTrash(paper)} className="gap-2 text-destructive">
          ...
          {/* Floating batch action bar for unfiled items */}
          <BatchBar
