...
function CardContent({
  ...
  return (
    <div className={`group relative min-w-0 rounded-sm border border-border/80 bg-card px-3.5 py-3 shadow-xs ${
      !isDragging ? "transition-all hover:border-border hover:shadow-md" : ""
    }`}>
            ...
            {currentUserId && onJoin && onLeave ? (
              ...
              >
                {isCurrentUserAssignee ? (
                  <UserMinus className="mr-2 h-4 w-4 text-foreground" />
                ) : (
                ...
                {metadataItems.map((item) => {
                  ...
                  return (
                    ...
                    >
                      ...
                      {hasText ? (
                        <span className="max-w-24 truncate whitespace-nowrap text-foreground">{item.text}</span>
                      ) : null}
                    </div>
                  );
                })}
