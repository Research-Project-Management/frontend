      ...
      if (!isExplanationCmd) {
        ...
        if (isActionableAiEditResponse(editResponse) && editResponse.intent !== "no_change") {
          ...
          if (!isEditSafe(validation, editResponse.intent)) {
            safetyWarning = `Ã¢Å¡Â Ã¯Â¸Â This edit affects ${Math.round(validation.replacementRatio * 100)}% of your file. Review carefully before applying.`;
          }
        ...
        <div className="px-3 pb-3 pt-2 border-t border-border/40 shrink-0 relative">
          ...
          <div className="relative rounded-2xl border border-border bg-background shadow-sm transition-shadow duration-300 focus-within:shadow-md focus-within:border-primary/30">
            <textarea
              ...
              placeholder={activeCommand ? `${activeCommand.label}: describe what you needÃ¢â‚¬Â¦` : "Ask Flux AIÃ¢â‚¬Â¦ or type / for commands"}
              rows={1}
