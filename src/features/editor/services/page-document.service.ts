...
export const PageDocumentService = {
  ...
  updateThumbnail: async (pageId: string, dataUrl: string) => {
    const res = await apiPut<{ page: Page }>(`/api/pages/${pageId}/thumbnail`, { pdfThumbnail: dataUrl });
    return res.page;
  },
...
export const PageFileService = {
  ...
  setMain: async ({ pageId, fileId }: { pageId: string; fileId: string }) => {
    const res = await apiPut<{ page: Page }>(`/api/pages/${pageId}/main-file`, { mainFileId: fileId });
    return res.page;
  },
};
