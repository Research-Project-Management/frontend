...
import { ChatModeProvider, useChatMode } from "@/features/chat-ai/hooks/useChatMode";
import { WikiChatFeatures } from "@/features/chat-ai";
import SideBar from "@/features/chat-ai/components/ai/layout/SideBar";
...
function ChatAiContent({ children }: { children?: React.ReactNode }) {
  ...
  return (
    <div className="flex-1 bg-background h-full flex overflow-hidden">
      {/* History sidebar â€” left */}
      <SideBar />

      {/* Main chat area */}
      <div className="flex-1 flex flex-col min-w-0 overflow-hidden relative">
        {/* Sources toggle button â€” floats top-right, only in wiki mode */}
      ...
      {/* Sources panel â€” right side, wiki mode only */}
      {showSources && sourcesOpen && (
