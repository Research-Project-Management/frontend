...
import { checkDuplicateFile, deleteItem } from '@/features/workspaces/projects/project-id/storage/services/storage.services';
...
export function useTopbar({
  ...
  const handleUploadFiles = useCallback(async (filesToUpload: File[], targetFolder: string | null) => {
    ...
    for (const file of filesToUpload) {
      try {
        const check = await checkDuplicateFile(projectId, file.name, targetFolder);
  ...
  }, [uploadFile, projectId, createFileRecord]);
