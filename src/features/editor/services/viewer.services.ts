...
export const compileLatex = async (payload: CompileLatexPayload): Promise<{ pdf: string; logs: string; synctex?: string }> => {
  return await apiPost<{ pdf: string; logs: string; synctex?: string }>("/api/latex/compile", payload);
};
