...
function branchNameMatches(node: TreeNode, query: string): boolean {
  return (
    ...
    node.children.some((child: any) => branchNameMatches(child, query))
  );
}
...
function CollectionTreeNode({
  ...
  const { state: { collectionPapers: papersResult, isLoadingCollection: isLoading } } = usePapers({ workspaceId, collectionId: node._id });
  const papers = (papersResult as any)?.papers ?? [];
  ...
  const visiblePapers = useMemo(() => {
    ...
    return papers.filter(
      (p: any) =>
        ...
        p.authors.some((a: any) => a.toLowerCase().includes(query)) ||
  ...
  const selectedCount = selectionScope.filter((p: any) => selected.has(p._id)).length;
  const allSelected = selectionScope.length > 0 && selectedCount === selectionScope.length;
  const someSelected = selectedCount > 0 && !allSelected;

  const hasChildren = node.children.length > 0;
  const childNameMatches = hasChildren && node.children.some((child: any) => branchNameMatches(child, query));
...
function LibraryTree({
  ...
  const { state: { collections }, actions: { create: createCollection } } = useCollections(workspaceId);
  const isLoading = false; // Simplified for this example
