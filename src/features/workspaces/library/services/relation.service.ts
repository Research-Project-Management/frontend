...
export const RelationService = {
  getRelated: (_scopeId: string, itemId: string) =>
    apiGet<{ relatedItems: RelatedItem[]; total: number }>(
      `/api/v1/library/items/${encodeURIComponent(itemId)}/relations`,
    ).then((res) => ({
  ...
  unlink: (_scopeId: string, itemId: string, targetItemId: string) =>
    apiDelete<{ message: string }>(
      `/api/v1/library/items/${encodeURIComponent(itemId)}/relations/${encodeURIComponent(targetItemId)}`,
    ),
};
