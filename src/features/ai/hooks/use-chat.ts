  ...
  const sendMessage = useCallback(
    async (text: string, projectId?: string, webSearchSites?: string[], intentHint?: string) => {
      ...
      try {
        ...
        // Server-Authoritative: The backend has already saved both user & assistant messages
        // If this was a new thread, navigate cleanly to the assigned chatId
        if (serverCreatedChatId && !chatId) {
          router.push(`/ai/${serverCreatedChatId}`);
        }
      } catch (error) {
