  ...
  return (
    <div className="overflow-hidden rounded-lg border border-border/55 bg-muted/20 text-xs">
      <div className="flex min-h-7 items-center gap-2 border-b border-border/45 bg-muted/35 px-2.5">
        ...
        {startLine > 0 && (
          <span className="ml-auto rounded-full border border-border/45 bg-background/70 px-1.5 py-0.5 font-sans text-xs text-muted-foreground/60">
            line {startLine}
          </span>
        )}
      </div>
      ...
      <div className="max-h-56 overflow-auto font-mono">
        {oldLines.map((line, i) => (
          ...
          >
            ...
            <span className="select-none border-r border-red-500/15 py-1.5 pr-2 text-right text-xs text-red-500/40">
              {startLine + i}
            </span>
        ...
        {newLines.map((line, i) => (
          ...
          >
            <span className="select-none py-1.5 text-center font-semibold">+</span>
            <span className="select-none border-r border-emerald-500/15 py-1.5 pr-2 text-right text-xs text-muted-foreground/45">
              {startLine + i}
            </span>
    ...
    <div className="w-full overflow-hidden rounded-lg rounded-tl-sm border border-border/60 bg-card">
      <div className="px-3.5 pt-3 pb-2.5 border-b border-border/35">
        <div className="mb-1.5 flex flex-wrap items-center gap-2">
          ...
          {editCount > 1 && (
            <span className="rounded-full border border-primary/25 bg-primary/10 px-1.5 py-px text-xs font-medium text-primary">
              {editCount} changes
            </span>
          )}
        </div>
        <div className="text-sm text-foreground/85 leading-relaxed [&_code]:break-all">
        ...
        <div className="px-3.5 pt-2.5 pb-1">
          ...
          {editCount > 1 && (
            <button
              ...
              className="mt-1.5 flex w-full items-center gap-1.5 rounded-md px-2 py-1 text-left text-xs font-medium text-muted-foreground/70 transition-colors hover:bg-secondary/45 hover:text-foreground outline-none"
            >
      ...
      <div className="flex flex-wrap items-center gap-2 px-3.5 py-2.5">
        {!isNoChange && (
          <button
            ...
            className="flex min-w-0 flex-1 items-center justify-center gap-1.5 whitespace-nowrap rounded-lg bg-primary px-3 py-1.5 text-sm font-semibold text-primary-foreground transition-colors hover:bg-primary/90 active:scale-[0.99] sm:flex-none outline-none"
          >
        ...
        <button
          type="button"
          onClick={onDismiss}
          className="flex min-w-0 flex-1 items-center justify-center gap-1.5 whitespace-nowrap rounded-lg border border-border/60 px-3 py-1.5 text-sm text-muted-foreground transition-colors hover:bg-secondary/50 hover:text-foreground sm:flex-none outline-none"
        >
